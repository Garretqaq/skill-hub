...
// 把来源（owner/repo 简写 / git URL）转成可点击的网页地址；无法识别返回 null
function toWebUrl(origin: string): string | null {
  const s = origin.trim()
  if (/^[\w.-]+\/[\w.-]+$/.test(s)) return `https://github.com/${s.replace(/\.git$/, '')}`
...
export default async function SkillDetailPage({ params }: PageProps) {
  ...
  const originLink = detail.entry.origin ? toWebUrl(detail.entry.origin) : null
              ...
              <div className="flex items-center gap-2 min-w-0">
                ...
                {detail.entry.origin ? (
                  <span className="min-w-0 flex items-center gap-1">
                    <span className="flex-shrink-0">来源仓库:</span>
                    {originLink ? (
                      <a href={originLink} target="_blank" rel="noopener noreferrer" className="text-cyan-400 hover:text-cyan-300 truncate">
                        {detail.entry.origin}
                      </a>
                    ) : (
                      <span className="text-zinc-400 truncate">{detail.entry.origin}</span>
                    )}
                  </span>
                ) : (
                  <span className="truncate">本地上传</span>
                )}
              </div>
            </div>
          </div>
